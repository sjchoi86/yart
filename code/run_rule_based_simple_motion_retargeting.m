function [q,mr_vec] = run_rule_based_simple_motion_retargeting(...
    chain_mocap,joi_mocap,chain_model,joi_model,jnames_ctrl,varargin)
%
...
mr_vec = [r2n_at_ease_rate,0.0,0.0,0.0,r2n_rate,n2s_rate,s2e_rate,e2h_rate]; % rule-based MR
...
ik = add_ik(ik,'joint_name',get_joint_name_from_joi_type(chain_model,joi_model,'ls'),...
    'p',pm_ls_mr,'IK_P',1);
if isempty(q_init)
    q_init = (ik.joint_limits_lower+ik.joint_limits_upper)/2; % median pose
end
